...
class DownloadHandler(FileSystemEventHandler):
    ...
    def map_extension(self, ext):
        ...
        code = [".c", ".java", ".class", ".cpp", ".cs", ".h", ".lua", ".m", ".py", ".sh", ".swift", ".vb", ".vcxproj"]
    ...
    def on_modified(self, event):
        ...
        for filename in os.listdir(self.folder):
            ...
            if extension and extension.upper() != '.CRDOWNLOAD': #arquivo de download em andamento
                ...
                file_exists = os.path.isfile(self.destination_folder + "/" + new_name)
                while file_exists:
                    self.i += 1
                    new_name = "{} ({}){}".format(sub_folder + base_name, self.i, extension)
                    file_exists = os.path.isfile("{}/{}".format(self.destination_folder, new_name))
                source = self.folder + "/" + filename
                new_destination = self.destination_folder + "/" + new_name
                self.notifier.show_toast("Python", "Movendo arquivo {} para {}".format(filename, self.destination_folder +'/'+ sub_folder), duration=8)
                shutil.move(source, new_destination)
            elif not extension: #folders
                ...
                while folder_exists:
                    ...
                    folder_exists = os.path.exists(new_destination)
                source = self.folder + "/" + filename
                self.notifier.show_toast("Python", "Movendo pasta {} para {}".format(filename, self.destination_folder +'/Folders'), duration=8)
                shutil.move(source, new_destination)
        self.running = False
